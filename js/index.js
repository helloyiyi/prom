...
$(function() {
	...
	$(".top_logo").click(function() {
		location.href = "http://prom.com/prom/";
	});
	...
	$(".mod_add").click(function() {
		...
		$.post(
			'http://prom.com/post/' + newModData.id, {
				data: newModData
			},
			"json");
	})
...
function refreshMembers(newuser) {
	...
	$.get(
		...
		function(response) {
			...
			$.post(
				'http://prom.com/post/logonMembers', {
					data: logonMembers
				});
		},
		"json");
...
function editCard(object) {
	...
	var id = object.parents(".card").attr("card_id");
...
function deleteCard(object) {
	...
	if (window.confirm("确定要删除该项目吗?")) {
		var card_id = object.parents(".card").attr('card_id');
		var project = project_array[card_id];

		// remove project from project_array
		delete project_array[project.id];

		// remove card from page
		object.parents('.card').remove();

		// remove the project on server
		$.get(
			'http://prom.com/remove/' + project.id);
	} else {}
}
...
function saveModTitle(object) {
	...
	$.post(
		'http://prom.com/post/' + foundMod.id, {
			data: foundMod
		});
}
